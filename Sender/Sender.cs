...
/*
* -------------------------------------- Public interfaces -------------------------
* CreateProxy -  An instance of the proxy is what the client uses to make calls on the server's remote service instance.  
* Connect - Create CommService and listener and start it
* sendMsgNotify - overridable message annunciator 
* sendAttemptNotify - overridable exception annunciator
* defineSendProcessing - defines SendThread and its operations
          ...
          {try
            {
              ...
              {
                if(Util.verbose) Console.Write("\n  sender sending message to service {0}", smsg.toUrl); // proxy already created so use it
                    ...
                    {  // create new proxy with Connect, save it, and use it
                        ...
                        else { sendMsgNotify(String.Format("could not connect to {0}\n", smsg.toUrl)); continue;}
              }
            }
            catch(Exception ex)
            {
